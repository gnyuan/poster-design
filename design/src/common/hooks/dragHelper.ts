...
export default class dragHelper {
  ...
  constructor() {
    ...
    window.addEventListener('mouseup', (e: any) => {
      ;(window as any).document
        .getElementById('app')
        .classList.remove('drag_active')
      const cl = e.target.classList
      if (
        e.target?.id === 'page-design-canvas' ||
        cl.contains('target') ||
  ...
  public start(e: any, finallySize: any) {
    if (!this.cloneEl) {
      ...
      ;(window as any).document
        .getElementById('app')
        .classList.add('drag_active') // 整个鼠标全局变成抓取
      // 选中了元素
  ...
  // 开始拖动初始化
  private init(
    { offsetX, offsetY, pageX, pageY, x, y }: any,
    { offsetWidth: width, offsetHeight: height }: any,
    finallySize: number,
    flag: any,
  ) {
    this.initial = {
