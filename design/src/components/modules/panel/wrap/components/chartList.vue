<template>
  <ul
    ref="listRef"
    class="img-list-wrap"
    :style="{ paddingBottom: isShort ? '15px' : '200px' }"
    ...
    <div class="list">
      ...
      >
        ...
        <template v-else>
          ...
          <div class="text-center">{{ item.wgtName }}</div>
        </template>
      </div>
    </div>
    <div v-if="!isDone" v-show="loading" class="loading">
...
export default defineComponent({
  ...
  setup(props, context) {
    ...
    watch(
      ...
      async (newList: any, oldList: any) => {
        ...
        let list = newList.filter(
          (v: any) => !newList.includes(v) || !oldList.includes(v),
        ) // difference
    ...
    const getInnerHeight = ({ height, listWidth, width }: any) =>
      (height * listWidth) / width
