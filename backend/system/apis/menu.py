...
from system.models import Menu, MenuButton, Users, Role
...
def route_menu_tree(request):
    ...
    if not token_user['is_superuser']:
        ...
        other_menuIds = Role.objects.filter(dept__in=user_departments).values_list('menu__id', flat=True)
        other_queryset = Menu.objects.filter(id__in=other_menuIds, status=1).values()

        queryset = queryset.union(other_queryset).values()
